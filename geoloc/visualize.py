...
def visualize_vlad_clusters(
	...
	vlad_color_img = cv2.resize(
		vlad_color_img, (query_image.shape[1], query_image.shape[2]), interpolation=cv2.INTER_NEAREST)
	...
	save_path = os.path.join(savedir, f"query_{query_ix:03d}_rot_{query_theta:03d}_vladvis.png")
	plt.savefig(save_path, dpi=DPI)
	plt.close(fig)
...
def visualize_similarity_heatmap_gurs(
	...
	for h, (dist, ind) in enumerate(zip(dists[0], inds[0])):
		...
		if h == 0:
			ax.plot(pr_col, pr_row, marker='x', color='blue', markersize=10, markeredgewidth=1)
	...
	np.save(os.path.join(savedir, f"heatmap_{query_ix:03d}_rot_{query_theta:03d}.npy"), heatmap)
	...
	save_path = os.path.join(savedir, f"query_{query_ix:03d}_rot_{query_theta:03d}_heatmap.png")
	plt.savefig(save_path, dpi=DPI)
	plt.close(fig)
...
def visualize_similarity_heatmap_visloc(
	...
	for i, (dist, ind) in enumerate(zip(dists[0], inds[0])):
		...
		if i == 0:
			ax.plot(col, row, marker='x', color='cyan', markersize=10, markeredgewidth=2)
