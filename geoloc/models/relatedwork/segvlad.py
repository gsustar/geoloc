...
class SegVLAD(AnyLocLikeModel):
    ...
    def post_init_setup(self, desired_height, desired_width):
        ...
        self.ind_matrix = torch.tensor(ind_matrix, device='cuda')
    ...
    def forward(self, x: torch.Tensor, idx: int):
        ...
        if self.order: 
            adjMat1_ind = nbrMasksAGGFastSingle(masks_seg, self.order)
        else:
            adjMat1_ind = None

        with torch.no_grad():
            dino_desc = self.backbone(img.unsqueeze(0))
        gd, _ = seg_vlad_gpu_single(self.ind_matrix, dino_desc, segMask1_ind, self.vlad.c_centers, self.desired_height, self.desired_width, desc_dim=1536, adj_mat=adjMat1_ind)
...
def seg_vlad_gpu_single(
    ind, dino_desc, segMask, c_centers, desired_height, desired_width, desc_dim=1536, adj_mat=None
):
    dh = desired_height // 14
    dw = desired_width // 14
