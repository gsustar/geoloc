"""
E2ResNet is modified based on ReResNet.
Implementation of ReResNet as in https://github.com/csuhan/ReDet.git
"""
...
class E2ResNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.out_type = FIELD_TYPE['regular'](self.gspace, middle_channels)
        ...
        if self.zero_init_residual:
            for m in self.modules():
                if isinstance(m, Bottleneck):
                    nn.init.constant_(m.norm3.weight, 0)  # enn.InnerBatchNorm wraps .bn
    ...
    def forward(self, x):
        ...
        out = self.gpool(outs[0]).tensor
        # out = outs[0]
        return out


    def train(self, mode=True):
        super(E2ResNet, self).train(mode)
        self._freeze_stages()
