...
class VLAD:
	...
	def __init__(self, num_clusters: int, 
				...
				soft_temp: float=1.0, cache_dir: Union[str,None]=None, 
	...
	def fit(self, train_descs: Union[np.ndarray, torch.Tensor, None], c_centers: Union[torch.Tensor, None]=None):
		"""
		...
		elif self.can_use_cache_vlad():
			print("Using cached cluster centers")
			self.c_centers = torch.load(
					f"{self.cache_dir}/c_centers.pt").to(device=self.device, dtype=torch.float32)
			self.kmeans.centroids = self.c_centers
			if self.desc_dim is None:
				self.desc_dim = self.c_centers.shape[1]
				print(f"Desc dim set to {self.desc_dim}")
	...
	def generate(self, query_descs: Union[np.ndarray, torch.Tensor],
		...
		if self.vlad_mode == 'hard':
			...
			one_hot = F.one_hot(labels, num_classes=self.num_clusters).float()  # GPU-friendly
