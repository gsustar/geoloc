...
class VLAD:
    ...
    def fit(self, train_descs: Union[np.ndarray, torch.Tensor, None]):
        ...
        if self.can_use_cache_vlad():
            ...
            self.c_centers = torch.load(f"{self.cache_dir}/c_centers.pt").to(
            ...
            if type(train_descs) == np.ndarray:
                train_descs = torch.from_numpy(train_descs).to(
                    device=self.device, dtype=torch.float32
                )
            if self.desc_dim is None:
    ...
    def generate(
        ...
        if self.vlad_mode == "hard":
            ...
            one_hot = F.one_hot(
                labels, num_classes=self.num_clusters
            ).float()  # GPU-friendly
            cd_sum = torch.einsum("qc,qcd->cd", one_hot, residuals)  # [c,d]
            if self.intra_norm:
                cd_sum = F.normalize(cd_sum, dim=1)
            un_vlad = cd_sum.flatten()
    ...
    def load_c_centers(self, c_centers: Union[torch.Tensor, str]):
        if isinstance(c_centers, str):
            c_centers = torch.load(c_centers)
        self.c_centers = c_centers.to(device=self.device, dtype=torch.float32)
        assert c_centers.shape[0] == self.num_clusters
        assert c_centers.shape[1] == self.desc_dim
        self.kmeans = fpk.KMeans(self.num_clusters, mode=self.mode)
        self.kmeans.centroids = self.c_centers
        if self.desc_dim is None:
            self.desc_dim = self.c_centers.shape[1]
