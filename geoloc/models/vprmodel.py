...
from .aggregators import SALAD, MixVPR
from .backbones import SaladDINOv2Backbone, MixVPRResNetBackbone

from ..config_parser import class_from_config, namespace_to_dict
...
# def get_backbone(model_name="salad", backbone_config={}):
# 	if "salad" in model_name.lower():
# 		return SaladDINOv2Backbone(**backbone_config)
# 	elif "mixvpr" in model_name.lower():
# 		return MixVPRResNetBackbone(**backbone_config)


# def get_aggregator(model_name="salad", agg_config={}):
# 	if 'mixvpr' in model_name.lower():
...
# 	elif 'salad' in model_name.lower():
...
class VPRModel(L.LightningModule):
    ...
    def __init__(
        ...
        self.loss_fn = class_from_config(self.loss_config)
        self.miner = class_from_config(self.miner_config)

        self.backbone = class_from_config(backbone_config)
        self.aggregator = class_from_config(aggregator_config)
        ...
        self.val_outputs = []  # For validation in Lightning v2.0.0
    ...
    def configure_optimizers(self):
        # optimizer = class_from_config(
        # 	params=self.parameters(),
        # 	**namespace_to_dict(self.optimizer_config)
        # )
        optimizer = class_from_config(self.optimizer_config, params=self.parameters())
        # scheduler = class_from_config(
        # 	**namespace_to_dict(self.scheduler_config)
        # 	optimizer=optimizer,
        # )
        scheduler = class_from_config(self.scheduler_config, optimizer=optimizer)
        return [optimizer], [scheduler]
