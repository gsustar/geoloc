...
class AnyLocDINOv3Backbone(nn.Module):
	...
	def __init__(
		...
		if self.facet == "token":
			self.fn_handle = self.backbone.backbone.layer[self.layer].\
	...
	def __call__(self, x: torch.Tensor):
		...
		res = einops.rearrange(res, "b (h w) c -> b c h w", h=latent_size[0], w=latent_size[1])
		return res
...
class AnyLocDINOv2Backbone(nn.Module):
	def __init__(
		...
		if self.facet == "token":
			self.fh_handle = self.backbone.blocks[self.layer].\
					register_forward_hook(
							self._generate_forward_hook())
		else:
			self.fh_handle = self.backbone.blocks[self.layer].\
...
class HFaceDINOv2Backbone(nn.Module):
	def __init__(self, pretrained_model_name_or_path="facebook/dinov2-giant",):
		super().__init__()
		self.pretrained_model_name_or_path = pretrained_model_name_or_path
		self.backbone = transformers.AutoModel.from_pretrained(pretrained_model_name_or_path)
		self.backbone = freeze(self.backbone)
		self.backbone.eval()
	...
	def forward(self, x: torch.Tensor):
		...
		x = x.transpose(1, 2).reshape(
			x.shape[0], -1, latent_size[0], latent_size[1]
		)
		return x

class HFaceDINOv3Backbone(nn.Module):
	def __init__(self, pretrained_model_name_or_path="facebook/dinov3-vitl16-pretrain-sat493m"):
		super().__init__()
		self.pretrained_model_name_or_path = pretrained_model_name_or_path
		self.backbone = transformers.AutoModel.from_pretrained(pretrained_model_name_or_path)
		self.backbone = freeze(self.backbone)
