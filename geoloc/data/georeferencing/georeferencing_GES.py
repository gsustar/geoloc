...
def compute_yaw_pitch_roll_from_GES(lat, lon, Rx, Ry, Rz):
	...
	psi, theta, phi = Rotation.from_euler('XYZ', [Rx, Ry, Rz-90], degrees=True).as_euler('ZYX', degrees=False) # NOTE: Rz-90 is necessary to obtain NED orientation
...
def compute_plane_ray_intersection(plane_point, plane_normal, ray_origin, image_corners_world):
	"""
...
def main():
	...
	for i, frame in enumerate(data["cameraFrames"]):
		...
		floor_point, n_e = construct_tangent_plane(lat, lon, ecef_point, height_above_ground)
		...
		geometry = Polygon([(pt[1], pt[0]) for pt in intersection_points if pt is not None])
		frame_info.append({
			"frame_index": i,
			"filename": filename,
			"lat": lat,
