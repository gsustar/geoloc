...
def parse_available_maps_endpoint(config: DownloaderConfig) -> None:
    response = requests.get(
        config.config.get("arcgis").get("arcgis_available_maps_endpoint")
    )
    ...
    if download_endpoint == "arcgis":
        ...
        for year in config.config.get("download_endpoints"):
            ...
            for endpoint in config.config.get("download_endpoints")[year]:
                if successfully_downloaded:
                    break
                tiles_dir = config.config.get("tiles_dir").format(
                ...
                successfully_downloaded = download_with_retry(
                    config, url, zoom, x, y, tile_name, tile_path, session
                )
        ...
        url = (
            ...
            .format(
                ...
                y=y,
                z=zoom,
                mapbox_access_token=config.config.get("mapbox").get(
                    "mapbox_access_token"
                ),
            )
        )
        ...
        if os.path.exists(tile_path):
            return
        url = config.config.get("ges").get("ges_endpoint").format(x=x, y=y, z=zoom)
        download_with_retry(config, url, zoom, x, y, tile_name, tile_path, session)
    else:
...
def download_with_retry(
    ...
    for attempt in range(max_attempts):
        ...
        try:
            if attempt > 0:
                print(
                    f"Retrying {attempt + 1} of {max_attempts} for tile {tile_name} at {zoom}_{x}_{y}..."
                )
                # Exponential backoff
