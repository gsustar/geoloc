...
class ALTOReferenceImages(torch.utils.data.Dataset, ResizeCenterCropMixin):
	...
	def __init__(self, root, round=1, split="Train", 
			  offset="offset_0_None", resize=None, center_crop=None):
		...
		self.new_root = os.path.join(
			self.root, self.round, self.split
		)
		...
		self.info = self.info[self.info["name"].str.contains(self.offset)].reset_index(drop=True)
...
class ALTOQueryImages(torch.utils.data.Dataset, ResizeCenterCropMixin):
	...
	def __init__(self, root, round=1, split="Val", resize=None, center_crop=None):
		...
		self.queries = get_sorted_imgpaths(
			os.path.join(self.new_root, "query_images")
		)
