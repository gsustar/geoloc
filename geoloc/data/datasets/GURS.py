...
class GURSDataset:
	...
	def __init__(self, root, tile_size=1000, border="Ljubljana", exclude_slo_border_tifs=True,
			  num_same_place=3, min_year="2011"):
		super().__init__()
		...
		self.min_east, self.min_north, self.max_east, self.max_north = self.disc_border_polygon.bounds

	def _load_info_csv(self, date_filter:str=None):
		info = pd.read_csv(os.path.join(self.root, "gurs_info.csv"), parse_dates=["DAT_POSN"])
	...
	def _load_border_multipolygon(self):
		...
		assert self.border in all_borders_gdf["NAZIV"].values, f"Border {self.border} not found in borders.geojson"
		self.border_polygon = all_borders_gdf[all_borders_gdf["NAZIV"] == self.border].geometry.values[0]
	...
	def _load_unique_tifs(self):
		...
		self.unique_tifs = self.info.drop_duplicates(subset="TIFNAME", keep="last").copy()
		self.unique_tifs["geometry"] = self.unique_tifs.apply(
			lambda row: box(row["BOUNDS_LEFT"], row["BOUNDS_BOTTOM"], row["BOUNDS_RIGHT"], row["BOUNDS_TOP"]),
			axis=1
		)
		self.unique_tifs = gpd.GeoDataFrame(self.unique_tifs, geometry="geometry", crs=self.crs)
	
	def _remove_slo_border_tifs(self):
		self.info = self.info[~self.info["IS_ON_BORDER"]]

	def _remove_outside_border_tifs(self):
		self.unique_tifs = self.unique_tifs[self.unique_tifs.intersects(self.border_polygon)]
	...
	def _get_all_relevant_tifs_for_window(self, win_bounds):
		...
		for i in range(num_rows):
			combined_dfs.append(
				pd.concat([tifs[tifname].iloc[[i]] for tifname in tifs.keys()], ignore_index=True)
			)
		return combined_dfs
...
class SequentialGURSDataset(GURSDataset, torch.utils.data.Dataset):
	...
	def _load_windows(self):
		...
		self.all_windows = self.all_windows[prep_disc_border_polygon.contains(self.all_windows.geometry)]
...
class SequentialWindowGURSDataset(SequentialGURSDataset):
	...
	def __getitem__(self, index):
		...
		for tifs in relevant_tifs:
			src_files = [rasterio.open(os.path.join(self.root, str(row["YEAR"]), row["AREA_CODE"], row["DATOTEKA"])) for _, row in tifs.iterrows()]
			mosaic, _ = merge(
				src_files,
				bounds=win_bounds.bounds
			)
			mosaic = torch.from_numpy(mosaic).float() / 255.0
			assert mosaic.shape == (3, self.tile_size, self.tile_size), f"Mosaic shape mismatch: {mosaic.shape}"
...
class GURSReferenceDataset(SequentialGURSDataset):
	...
	def __getitem__(self, index):
		...
		src_files = [rasterio.open(os.path.join(self.root, str(row["YEAR"]), row["AREA_CODE"], row["DATOTEKA"])) for _, row in relevant_tifs.iterrows()]
		mosaic, _ = merge(
			src_files,
			bounds=win_bounds.bounds
		)
...
class TrainGURSDataset(GURSDataset, torch.utils.data.Dataset):
	...
	def __init__(self, *args, **kwargs):
		super().__init__(*args, **kwargs)
		assert self.tif_h % self.tile_size == 0, "TIF height must be divisible by tile size"
		assert self.tif_w % self.tile_size == 0, "TIF width must be divisible by tile size"
	...
	def __getitem__(self, index):
		...
		for i, r_tif in relevant_tifs.iterrows():
			...
			if tiffile not in self.tif_cache:
				self.tif_cache[tiffile] = rasterio.open(os.path.join(self.root, str(r_tif["YEAR"]), r_tif["AREA_CODE"], r_tif["DATOTEKA"]))
			...
			assert data.shape == (3, self.tile_size, self.tile_size), f"Data shape mismatch: {data.shape}"
			datas.append(data)
...
class TifAwareShuffleSampler(torch.utils.data.Sampler):
	...
	def _get_shuffled_indices(self):
		...
		for i in range(0, self.num_tifs, self.N):
			...
			win_ixs = np.array([indices[j * self.num_windows_per_tif:(j + 1) * self.num_windows_per_tif] for j in tif_ixs]).flatten()
