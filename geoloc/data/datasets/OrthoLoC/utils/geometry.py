...
def crop_rasters(point_map: np.ndarray, dsm: np.ndarray, dop: np.ndarray, covisibility_ratio: float,
    ...
    if plot and dop_copy is not None and point_map_in_dsm_mask_copy is not None:
        intersection_mask_rgb = image.mask_to_rgba(intersection_mask & dsm_mask, to_rgb('blue'))
        point_map_in_dsm_mask_rgb = image.mask_to_rgba(point_map_in_dsm_mask_copy & dsm_mask, to_rgb('red'))
        dsm_mask_rgb = image.mask_to_rgba(dsm_mask_copy & dsm_mask, to_rgb('yellow'))
