...
class CosKernel(nn.Module):  # similar to softmax kernel
    def __init__(self, T, learn_temperature=False):
        ...
        if self.learn_temperature:
            ...
            # self.register_buffer("T", torch.tensor(float(T)), persistent=False)
            # self.T = T
...
def conjugate_gradient(A, b, num_iters=20, eps=1e-6):
    """
    Solve A @ x = b iteratively using only matmuls.
    A: (batch, n, n) symmetric positive definite
    b: (batch, n, d)
    """
    ...
    for _ in range(num_iters):
        Ap = A @ p
        alpha = r_dot / ((p * Ap).sum(dim=1, keepdim=True) + eps)
        x = x + alpha * p
        r = r - alpha * Ap
        r_dot_new = (r * r).sum(dim=1, keepdim=True)
        beta = r_dot_new / (r_dot + eps)
        p = r + beta * p
        r_dot = r_dot_new
    return x
    ...
    def forward(
        ...
        for new_scale in all_scales:
            ...
            if new_scale in self.proj:
                ...
                # )
                autocast_device, autocast_enabled, autocast_dtype = get_autocast_params(
                    f1_s.device, f1_s.device.type == "cuda", self.amp_dtype
                )
                with torch.autocast(
                    autocast_device, enabled=autocast_enabled, dtype=autocast_dtype
                ):
...
class RegressionMatcher(nn.Module):
    ...
    def sample(
        ...
        # good_samples = torch.topk(
        #     certainty, 
        #     min(expansion_factor * num, len(certainty)), 
        #     sorted=False,
        # ).indices
