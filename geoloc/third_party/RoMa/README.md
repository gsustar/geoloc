...
- removed code I don't need for inference
- moved `self.train(False)` from `.match()` to `__init__()` in `matcher.py [RegressionMatcher]`
- changed `str(f1_s) == "cuda"` to `f1_s.device.type` in `matcher.py [Decoder.forward()]`
- removed `torch.tensor()` call in CosKernel `forward`
<!-- - replaced `torch.linalg.cholesky` with `conjugate_gradient` method -->
- remove `torch.get_float32_matmul_precision() != "highest"` since it's only relevant for training
<!-- - replaced `cdist` in `kde` with onnx compatible cdist -->
- Now using normal `Attention` for compatibility with onnx
<!-- - Replaced first `.multionmial` in sampling with topk -->
<!-- - removed `multinomial` sampling and used Gumbel-max trick and topk -->
