...
def polygon_intersection_metrics(qry_polygon: Polygon, ref_polygon: Polygon, tp_threshold=0.25):
	inter = qry_polygon.intersection(ref_polygon)
	qry_ioa = inter.area / qry_polygon.area
	ref_ioa = inter.area / ref_polygon.area
	area_imbalance = max(qry_polygon.area, ref_polygon.area) / min(qry_polygon.area,ref_polygon.area)
	is_tp = max(qry_ioa, ref_ioa) >= tp_threshold
	return qry_ioa, ref_ioa, area_imbalance, is_tp


def calculate_distances(qry, benchmark_top_k, inds, qry_image_dataset, ref_image_dataset):
	...
	gdists = np.array(
		[distance_between_points(
			qry_point, ref_point, ref_image_dataset.crs) 
