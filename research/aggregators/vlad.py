...
class VLAD:
	"""
		...
		- vlad_mode:        Mode for descriptor assignment (to cluster
							...
							{'soft', 'hard'}
		- minibatch:        Batch size of MinibatchKmeans algorithm
	...
	def __init__(
		...
		self.kmeans_type = str(kmeans_type).lower()
		assert self.kmeans_type in ['minibatch', 'fpk', 'sklearn']
		
		if self.kmeans_type == "minibatch":
			self.kmeans = sklearn.cluster.MiniBatchKMeans(
	...
	def fit(self, train_descs: Union[np.ndarray, torch.Tensor, None]):
		...
		"""
		train_descs = train_descs.detach().cpu()
		# train_descs = ein.rearrange(train_descs, "d L -> L d")
		...
		if self.can_use_cache_vlad():
			...
			if self.kmeans_type == "fpk":
				self.kmeans.centroids = self.c_centers
			else:
				self.kmeans.cluster_centers_ = self.c_centers.cpu().numpy()
			...
			if self.kmeans_type == "fpk":
				...
				train_descs = train_descs.numpy()
				if self.kmeans_type == "minibatch":
					self.kmeans.partial_fit(train_descs)
				else:
					self.kmeans.fit(train_descs)
				self.c_centers = torch.from_numpy(
	...
	def fit_and_generate(self, 
		...
		"""
			...
			- train_descs:  Training image descriptors of shape
							[num_imgs, desc_dim, num_descs]. There are
		...
		"""
		# Generate vocabulary
		all_descs = ein.rearrange(train_descs, "N d L -> d (N L)")
		self.fit(all_descs)
	...
	def __call__(self, x: torch.Tensor) -> torch.Tensor:
		""" Input should be [Ni, d, L] """
		return self.generate_multi(x)