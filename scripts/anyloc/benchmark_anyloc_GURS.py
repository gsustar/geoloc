...
from geoloc.visualize import visualize_top_k_retrieved, visualize_vlad_clusters, visualize_similarity_heatmap_gurs
...
@torch.no_grad()
def benchmark_anyloc_gurs(vdbdir, traj_config):
		...
		for i, qry in enumerate(tqdm(qry_image_dataset)):
			...
			if traj_config.VISUALIZE_TOP_K:
				visualize_top_k_retrieved(
					...
					rotexp_thetas=rotexp_thetas, query_theta=theta
				)
			if traj_config.VISUALIZE_VLAD:
			...
			if traj_config.VISUALIZE_HEATMAP:
				...
				visualize_similarity_heatmap_gurs(
					...
					query_northing=qry_north, query_easting=qry_east, 
					savedir=savedir, rotref_exp=config.ROTREF_EXP
				)
	tracemalloc.stop()
...
def main():
	...
	args = parser.parse_args()
	traj_config = load_config(args.traj_config)
	benchmark_anyloc_gurs(args.vdbdir, traj_config)
