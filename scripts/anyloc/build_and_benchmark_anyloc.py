...
from fit_anyloc import fit_anyloc
...
def main():
    ...
    fit_config_path = f"configs/anyloc/{args.dataset}/fit_anyloc_{args.dataset}.yaml"
    build_vdb_config_path = (
        f"configs/anyloc/{args.dataset}/build_vdb_anyloc_{args.dataset}.yaml"
    )
    benchmark_config_path = (
        f"configs/anyloc/{args.dataset}/benchmark_anyloc_{args.dataset}.yaml"
    )
    fit_config = load_config(fit_config_path)
    build_vdb_config = load_config(build_vdb_config_path)
    benchmark_config = load_config(benchmark_config_path)

    base_savedir = fit_config.savedir
    if args.dataset == "VPAir":
        fit_anyloc(fit_config)
        build_vdb_anyloc(base_savedir, build_vdb_config)
        ...
        for flight_idx in flight_idxs:
            ...
            try:
                fit_anyloc(fit_config)
                build_vdb_anyloc(new_savedir, build_vdb_config)
