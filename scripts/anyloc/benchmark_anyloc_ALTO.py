# CUDA_VISIBLE_DEVICES=0 PYTHONPATH=. DEBUG=2 python scripts/anyloc/benchmark_anyloc_ALTO.py --vdbdir /storage/datasets/AerialLoc/Drone2Sat/vdbs/anyloc/ALTO/paper --traj_config configs/anyloc/ALTO/benchmark_anyloc_ALTO.yaml
...
@torch.no_grad()
def benchmark_anyloc_alto(vdbdir, traj_config):
    ...
    loaddir = fit_config.savedir  # savedir from build_vdb_anyloc is now loaddir
    ...
    for i, qry in enumerate(tqdm(qry_image_dataset)):
        ...
        x = anyloc(
            image.unsqueeze(0), return_residuals=traj_config.VISUALIZE_VLAD
        )  # Add batch dimension before calling anyloc
...
def main():
    ...
    benchmark_anyloc_alto(args.vdbdir, traj_config)
