...
def fit_anyloc(config, batchsize=16):
    ...
    model = class_from_config(config.model).to(device)
    ...
    for i, batch in enumerate(tqdm(ref_image_dataloader)):
        ...
        x = model.backbone(image)
        x = einops.rearrange(x, "b c h w -> b (h w) c")
        features.append(x.detach())
    features = torch.vstack(features)

    if config.vlad_prefitted:
        model.vlad.load_c_centers(config.vlad_prefitted)
        features = model.vlad.generate_multi(features)
        model.vlad.save()  # Manually save `c_centers` (usually this happens during `fit`)
    else:
        print("Fitting VLAD features...")
        assert model.vlad.cache_dir is not None
        features = model.vlad.fit_and_generate(features)

    if model.pca is not None:
        print("Fitting PCA...")
        features = model.pca.fit_transform(features.cpu())
        with open(os.path.join(savedir, "pca.pkl"), "wb") as f:
            pickle.dump(model.pca, f)
        print(f"PCA components: {model.pca.components_.shape}")
